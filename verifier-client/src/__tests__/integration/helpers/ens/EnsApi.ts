...
export class EnsApi {
  ...
  constructor(deps: { ethersProvider: ethers.providers.Provider }) {
    this.ethersProvider = deps.ethersProvider;
  }

  async init(): Promise<void> {
    const signer = new ethers.Wallet(process.env.REGISTRY_AUTHORITY_PRIVATE_KEY!, this.ethersProvider);
