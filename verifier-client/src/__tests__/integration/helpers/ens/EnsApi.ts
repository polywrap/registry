...
export class EnsApi {
  ...
  async registerDomainName(domainOwner: Signer, domain: EnsDomain): Promise<void> {
    await this.testEthRegistrarL1.register(domain.labelHash, await domainOwner.getAddress(), 10 * 60);
    const ownedRegistry = this.ensRegistryL1.connect(domainOwner);

    const tx = await ownedRegistry.setResolver(domain.node, this.testPublicResolverL1!.address);
  ...
  async getPolywrapOwner(domain: EnsDomain): Promise<string> {
    return await this.testPublicResolverL1.text(domain.node, POLYWRAP_OWNER_RECORD_NAME);
  }
}