...
import { OwnershipBridgeLink__factory, PackageOwnershipManager__factory, PolywrapRegistry__factory, VerificationRootBridgeLink__factory, VersionVerificationManager__factory } from "../../../../typechain";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const registryL1 = PolywrapRegistry__factory.connect((await hre.deployments.get("PolywrapRegistryL1")).address, signer);
  const versionVerificationManagerL1 = VersionVerificationManager__factory.connect(
    (await hre.deployments.get("VersionVerificationManagerL1")).address, signer
  );
  const packageOwnershipManagerL1 = PackageOwnershipManager__factory.connect(
    (await hre.deployments.get("PackageOwnershipManagerL1")).address, signer
  );
